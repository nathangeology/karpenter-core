...
var _ = Describe("Performance", Label(debug.NoWatch), func() {
	Context("Do Not Disrupt Performance Test", func() {
		It("should efficiently scale three deployments and test disruption protection", func() {
			...
			scaleOutReport, err := ReportScaleOutWithOutput(env, "Do Not Disrupt Performance Test", 1100, 15*time.Minute, "do_not_disrupt_scale_out", sizeClassLockThreshold, consolidateWhen, decisionRatio)
			...
			consolidationReport, err := ReportConsolidationWithOutput(env, "Do Not Disrupt Consolidation Test", 1100, 600, initialNodes, 25*time.Minute, "do_not_disrupt_consolidation", sizeClassLockThreshold, 0, consolidateWhen, decisionRatio)
