...
var _ = Describe("Performance", Label(debug.NoWatch), func() {
	Context("Drift Performance", func() {
		It("should efficiently handle drift replacement of pods with topology constraints", func() {
			...
			initialReport, err := ReportScaleOutWithOutput(env, "Drift Test Initial Deployment", 600, 20*time.Minute, "drift_initial_deployment", sizeClassLockThreshold, consolidateWhen, decisionRatio)
