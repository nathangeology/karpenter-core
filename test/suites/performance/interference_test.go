...
var _ = Describe("Performance", Label(debug.NoWatch), func() {
	Context("Self Anti-Affinity Deployment Interference", func() {
		It("should efficiently scale two deployments with self anti-affinity", func() {
			...
			scaleOutReport, err := ReportScaleOutWithOutput(env, "Self Anti-Affinity Performance Test", 500, 15*time.Minute, "self_antiaffinity_scale_out_small", sizeClassLockThreshold, consolidateWhen, decisionRatio)
			...
			interferenceReport, err := ReportScaleOutWithOutput(env, "Self Anti-Affinity scale out Test", 750, 5*time.Minute, "self_antiaffinity_interference", sizeClassLockThreshold, consolidateWhen, decisionRatio)
			...
			consolidationReport, err := ReportConsolidationWithOutput(env, "Interference Consolidation Test", 750, 600, initialNodes, 15*time.Minute, "self_antiaffinity_interference_consolidation", sizeClassLockThreshold, 0, consolidateWhen, decisionRatio)
