...
var _ = Describe("Performance", Label(debug.NoWatch), func() {
	Context("Host Name Spreading Deployment Reg", func() {
		It("should efficiently scale two deployments with host name topology spreading", func() {
			...
			scaleOutReport, err := ReportScaleOutWithOutput(env, "Host Name Spreading Performance Test", 1000, 15*time.Minute, "hostname_spread_scale_out", sizeClassLockThreshold, consolidateWhen, decisionRatio)
			...
			consolidationReport, err := ReportConsolidationWithOutput(env, "Hostname Spread Consolidation Test", 1000, 700, initialNodes, 30*time.Minute, "hostname_spread_consolidation", sizeClassLockThreshold, 0, consolidateWhen, decisionRatio)
