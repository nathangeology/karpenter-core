...
var _ = Describe("Ranking", func() {
	...
	Context("Group D: Disrupted + PDB-blocked nodes", func() {
		...
		It("should not classify disrupted node without PDB-blocked pods as Group D", func() {
			nodeClaims, nodes := test.NodeClaimsAndNodes(2, v1.NodeClaim{
				ObjectMeta: metav1.ObjectMeta{Labels: map[string]string{v1.NodePoolLabelKey: nodePool.Name}},
				Status:     v1.NodeClaimStatus{Allocatable: corev1.ResourceList{corev1.ResourceCPU: resource.MustParse("4"), corev1.ResourceMemory: resource.MustParse("8Gi")}},
			})
			...
			// Both should be in the normal tier (no Group D) since there's no PDB blocking
			for _, r := range ranks {
				Expect(r.HasDoNotDisrupt).To(BeFalse())
			}
		})
		...
		It("should place Group D below Group A (drifted) in ordering", func() {
			nodeClaims, nodes := test.NodeClaimsAndNodes(3, v1.NodeClaim{
				ObjectMeta: metav1.ObjectMeta{Labels: map[string]string{v1.NodePoolLabelKey: nodePool.Name}},
				Status:     v1.NodeClaimStatus{Allocatable: corev1.ResourceList{corev1.ResourceCPU: resource.MustParse("4"), corev1.ResourceMemory: resource.MustParse("8Gi")}},
			})
			...
			// Node 0: disrupted + PDB-blocked (Group D)
			nodes[0].Spec.Taints = append(nodes[0].Spec.Taints, v1.DisruptedNoScheduleTaint)
			ExpectApplied(ctx, env.Client, nodes[0])
			ExpectApplied(ctx, env.Client, test.Pod(test.PodOptions{
				ObjectMeta: metav1.ObjectMeta{Labels: map[string]string{"app": "blocked"}},
			...
			var groupDRank, groupARank, groupBRank int
			for _, r := range ranks {
				switch r.Node.Node.Name {
				case nodes[0].Name:
					groupDRank = r.Rank
			...
			Expect(groupDRank).To(BeNumerically("<", groupARank))
