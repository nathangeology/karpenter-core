...
// Group D (lowest cost): Disrupted + PDB-blocked nodes — sorted by pod count ascending
// Group A (low cost):    Drifted nodes — sorted by pod count ascending
// Group B (middle):      Normal nodes — sorted by pod count ascending
// Group C (highest):     Do-not-disrupt nodes — sorted by pod count ascending
...
// D. DisruptedBlocked — has karpenter.sh/disrupted taint AND at least one pod blocked by a PDB
// A. Drifted — NodeClaim has ConditionTypeDrifted=True (but not disrupted+blocked)
// B. Normal — not drifted, no do-not-disrupt pods
// C. DoNotDisrupt — has do-not-disrupt pods (regardless of drift)
