...
func TestBalancedScoreThreshold(t *testing.T) {
	...
	// All disruption comes from the candidate's pods.
	...
	// threshold for k=2 is 0.5; high score should exceed it
	if highScore < 0.5 {
...
// TestBalancedScoreLogging verifies that checkBalancedScore sets ReasonOverride
// to DisruptionReasonBalanced when the score exceeds the threshold.
func TestBalancedScoreLogging(t *testing.T) {
	ctx := context.Background()
	pod := &corev1.Pod{ObjectMeta: metav1.ObjectMeta{Name: "p1", Namespace: "default"}}
	np := balancedNodePool(v1.ConsolidationPolicyBalanced, int32Ptr(2))
	...
	result, ok := c.checkBalancedScore(ctx, cmd, allCandidates, "single")
	// DELETE move with 100% savings should pass
	if !ok {
		t.Fatal("expected checkBalancedScore to approve the move")
	}
	if result.ReasonOverride != v1.DisruptionReasonBalanced {
		t.Errorf("ReasonOverride = %q, want %q", result.ReasonOverride, v1.DisruptionReasonBalanced)
	}
}
