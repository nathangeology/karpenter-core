...
This controller belongs inside Karpenter rather than as a standalone component or generic Kubernetes feature for three reasons.

**The ranking is Karpenter's consolidation signal, not a generic heuristic.** The three-tier partitioning (drifted → normal → do-not-disrupt) and within-tier ordering (pod count ascending) mirror Karpenter's own consolidation candidate selection. A standalone controller would need to reverse-engineer this logic and track Karpenter's internal evolution. Co-location makes the ranking authoritative: it uses the same `state.Cluster` informer cache, the same drift condition semantics, and the same `do-not-disrupt` annotation interpretation that the consolidation controller uses. When the ranking and the consolidation decision come from the same state, they cannot diverge.

**The ranking must evolve with consolidation scoring.** Today the ranking uses pod count. When balanced consolidation ([designs/balanced-consolidation.md](balanced-consolidation.md)) ships, the ranking should incorporate the move score — nodes with the highest score (cheapest to consolidate relative to savings) should drain first. That integration is a one-line change when both controllers share a process and a state store. As a standalone controller, it would require a new API surface or a shared database to communicate scores, adding latency and failure modes.

**Karpenter already pays the operational cost.** RBAC, Helm chart, release lifecycle, CRD watches, and node/pod informers already exist. Adding a reconcile loop inside the existing process costs one goroutine and zero additional infrastructure. A standalone controller doubles the watch load on pods and nodes, requires its own RBAC configuration, and introduces a new failure domain that operators must monitor independently.

#### Placement constraints are complementary, not competing

This ranking is a node-level consolidation heuristic. It does not attempt to reason about topology spread constraints, pod affinity/anti-affinity, or zone distribution — and neither does the current RS controller spreading heuristic it replaces. The two systems address different concerns:

- **Karpenter ranks entire nodes** for autoscaling decisions. The atomic unit is "can I remove this node?" Individual pod ranking would fragment the signal needed for node-level consolidation.
- **kube-scheduler enforces placement** when pods are created. Topology spread, affinity, and zone distribution are scheduling-time constraints that the scheduler satisfies when placing replacement pods.

The RS controller's current spreading heuristic doesn't consider topology constraints either — it simply distributes deletions evenly across nodes, which is equally unaware of zone topology. This controller substitutes a different heuristic (consolidation-aware) with the same level of placement awareness (none). Placement correctness is maintained by the scheduler on the create path, not by the RS controller on the delete path.

Over time, the ranking can evolve to incorporate placement-relevant signals (e.g., deprioritizing nodes in under-represented zones) as additive factors within the existing framework. Co-location with Karpenter's `state.Cluster` makes this evolution straightforward — the cluster state already tracks node topology.
...
Additional examples (partial drain convergence, drifted node priority, do-not-disrupt partitioning) are in [Appendix E](#appendix-e-additional-examples).
...
A complementary approach modifies the RS controller's pod deletion algorithm itself ([KEP: ReplicaSet Consolidation-Aware Scale-In Strategy, kubernetes/enhancements#5982](https://github.com/kubernetes/enhancements/issues/5982)). This RFC ships value immediately via annotations; the KEP provides a zero-overhead long-term replacement. The two coexist cleanly — annotations take precedence in the existing sort order. The recommended path: ship this RFC now, pursue the KEP in parallel, deprecate the annotation controller once the KEP reaches GA. See [Appendix F](#appendix-f-rs-controller-kep-details) for the full comparison.
...
Neither this proposal nor the current RS spreading heuristic considers topology spread constraints, pod affinity, or zone distribution — both are simple heuristics optimizing for different goals (consolidation vs. even distribution). The scheduler enforces placement correctness when creating replacement pods. Operators with strict spreading requirements can leave the feature disabled. See [Appendix G](#appendix-g-placement-and-spreading-analysis) for the full analysis.
...
Annotations are the v1 mechanism, not the end state. Known limitations (last-writer-wins, no multi-contributor support, timing constraints) motivate three future directions: a dedicated `PodDisruptionPreference` API, scheduler library integration, and eventual deprecation of the annotation controller. See [Appendix H](#appendix-h-future-work-details) for details.
...
## Appendix F: RS Controller KEP Details
...
## Appendix G: Placement and Spreading Analysis
