...
Group D always receives the absolute lowest deletion costs, then Group A, Group B the middle range, and Group C the highest. The ReplicaSet controller removes pods from disrupted+blocked nodes first (unblocking in-progress drains), then drifted nodes, then normal consolidation targets, and protected nodes last.
...
4. Partitions nodes into Group D (disrupted + PDB-blocked), Group A (drifted, no do-not-disrupt pods), Group B (not drifted, no do-not-disrupt pods), and Group C (has do-not-disrupt pods)
5. Ranks each group independently by pod count ascending
6. Assigns sequential integer ranks starting at -n (where n is the total number of Karpenter-managed nodes) for Group D, continuing for Group A, then Group B, then Group C
...
│  │  2. Rank Group D (disrupted+blocked) │                       │
│  │  3. Rank Group A (drifted)           │                       │
│  │  4. Rank Group B (normal)            │                       │
│  │  5. Rank Group C (do-not-disrupt)    │                       │
...
### Example: Drifted node drains first via four-tier ranking
...
**Without four-tier ranking:** Node A would compete equally with Nodes B and C for deletions. The drifted node might not drain first, delaying drift compliance while providing no additional consolidation benefit.

### Example: Disrupted + PDB-blocked node gets highest drain priority
...
**Scale from 9 to 6 replicas.** The ReplicaSet controller sees all 3 pods on Node A have the lowest deletion cost (-3). It removes pods from Node A first. Once a replica pod is removed, the PDB's `DisruptionsAllowed` increases for the remaining pods, unblocking Karpenter's drain. The disruption that was stalled completes without additional Karpenter-initiated evictions.

**Why Group D exists above Group A:** A disrupted+blocked node represents a stalled operation — Karpenter has already committed resources (replacement node may be booting, taint applied, drain initiated) but cannot complete. Every second it remains blocked is wasted capacity. Drifted nodes also need action but aren't actively stalled — they can wait one more scale-down cycle.
...
**RankingEngine (ranking.go):** Ranks nodes by pod count (mirroring Karpenter's consolidation candidate sorting). Before ranking, it partitions nodes into four groups: Group D (disrupted + PDB-blocked: has `karpenter.sh/disrupted` taint AND at least one pod blocked by a PDB with `DisruptionsAllowed=0`), Group A (drifted, no do-not-disrupt pods), Group B (not drifted, no do-not-disrupt pods), and Group C (has at least one do-not-disrupt pod). Each group is ranked independently by pod count ascending. Group D gets the lowest ranks, then Group A, Group B the middle range, and Group C the highest (continuing sequentially). Uses `sort.SliceStable` with deterministic tie-breaking by node name. Disrupted state is detected from the `karpenter.sh/disrupted` taint on the Node. Drift status is read from `ConditionTypeDrifted` on the node's `StateNode`. PDB blocking is determined by listing PDBs and checking `DisruptionsAllowed=0` against node pods.
